...
function add_bar_sales_item($selected_item, $size, $stock, $store_ref, $price, $bar_sales_no, $item_total, $order_type, $ref_no, $booking_ref){
	include 'conf/config.php';
	include 'conf/opendb.php';

	$date = date("Y-m-d");

	$query = "INSERT INTO bar_sales_has_items (id, liquor_id, liquor, store_ref, size, price, date, bar_sales_no, quantity, user_name, total, order_type, ref_no, booking_ref)
	VALUES ('', '$selected_item', '$stock', '$store_ref', '$size', '$price', '$date', '$bar_sales_no', '1', '$_SESSION[user_name]', '$item_total', '$order_type', '$ref_no', '$booking_ref')";
...
function update_stock_bar_sales($bar_sales_no){
	...
	$result=mysqli_query($conn, "SELECT * FROM bar_sales_has_items WHERE  bar_sales_no='$bar_sales_no' AND cancel_status='0'");
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
	{
		if($row['size']=="full"){
			$qty=$row['quantity'];
		}
		elseif($row['size']=="25 ml"){
			$qty=(25*$row['quantity'])/750;
		}
		else{
			$qty=(50*$row['quantity'])/750;
		}
		$info=get_store_info($row[store_ref]);
		$new_qty=$info['qty']-$qty;
		
		$query = "UPDATE store SET
		qty='$new_qty'
		WHERE item='$row[store_ref]'";
